...
Compressed ICP configuration with target signals, geographies, and scoring weights

**research_context_compressed**
Client information (name, services, differentiators)

**seed** *(optional)*
Specific entity name or signal to research

**hint** *(optional)*
Direction from batch composer for search focus

**attempt_log** *(optional)*
Previous failed attempts (must pivot strategy if provided)

**exclude_domains** *(optional)*
Array of domains already in pipeline to skip
...
- `objective` - Summary of search intent
- `queries` - Array of 3-5 search query objects
- `search_strategy` - Strategy summary with primary targets
- `domains_to_exclude` - Passthrough of excluded domains
...
**Make.com Setup:**
- Variables section → HTTP POST body to API (if using middleware)
- Main template → AI module prompt field
- Map variables in Make.com: `{{icp_config_compressed}}` → Module output

**Output Usage:**
- queries array → Passed to 2_SIGNAL_DISCOVERY for execution
- domains_to_exclude → Passed forward to prevent duplicate research
