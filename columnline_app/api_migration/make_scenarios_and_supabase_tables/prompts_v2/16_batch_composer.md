# Batch Composer (Seed Generation)
...
**batch_strategy**
Client's batch strategy config with distribution, focus areas, seed generation rules

**icp_config**
Full (thorough) ICP configuration - NOT compressed (this is high-impact step)

**industry_research**
Full (thorough) industry research - NOT compressed

**seed_pool** *(optional)*
Pre-scraped seed companies (future feature - may be empty)

**last_batch_hints** *(optional)*
Previous batch dossiers to avoid duplicates

**recent_feedback** *(optional)*
Client feedback to adjust seed selection
...
- `seeds` - Array of seed objects for pipeline runs
- `seeds_generated` - Count of seeds
- `distribution_achieved` - Actual distribution by project type
- `distribution_match` - Boolean indicating if target met
- `estimated_avg_score` - Average predicted ICP score

---

## Integration Notes

**Model:** gpt-4.1 (sync, 3-5 min for web research in discovery mode)

**Important:** This step uses FULL (thorough) ICP and industry research configs, NOT compressed versions. Batch planning is high-impact and worth the extra tokens.

**Purpose:** Pre-pipeline seed generation. Creates inputs for parallel dossier runs.

**Next Steps:**
- Each seed becomes input to MAIN_DOSSIER_PIPELINE
- Pipeline runs in parallel (10 seeds = 10 simultaneous dossier runs)
- Seeds provide hint data that improves Signal Discovery step efficiency

**Execution Patterns:**
- **Small batches (1-3)**: Client wants targeted research on specific companies
- **Medium batches (4-7)**: Weekly batch for ongoing pipeline
- **Large batches (8-10)**: Monthly batch for comprehensive market coverage

**Feedback Loop:**
- After batch delivers, client reviews dossiers
- Client feedback flows back into next batch's recent_feedback parameter
- Continuous improvement: "more [geography from ICP]", "higher scores", "only EPCM awards", etc.
