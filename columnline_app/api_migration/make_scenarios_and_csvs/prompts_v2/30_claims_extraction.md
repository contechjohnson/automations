...
- `claims` - Array of context-rich claim objects
...
- `extraction_metadata` - Extraction details including context preservation level
...
**Purpose:** Transform research narratives into context-rich claims that downstream steps can use to understand not just WHAT happened, but WHY it matters and HOW things connect.

**Philosophy Change:**
- **OLD**: Extract atomic facts, lose context
- **NEW**: Extract mini-narratives, preserve meaning

**Flow:**
1. Research step produces narrative (long-form)
2. Claims Extraction preserves context → claims array
3. Claims flow to downstream steps WITH their context intact
4. Section writers have full story, not just isolated facts

**Why Context Matters:**
- Downstream LLMs need to understand relationships
- Strategic meaning informs positioning
- Temporal connections show urgency
- Causal relationships explain "why"

**Custom Rules:**
- Clients can still provide custom extraction rules
- Example: "Preserve all budget evolution context"
- Example: "Keep timeline relationships for all milestones"
