...
- `routing_plan` - Array of section generation instructions
- `sections_to_generate` - Count of sections that will be generated
- `sections_placeholder` - Count of sections that need placeholders
- `data_quality_flags` - Warnings about missing or partial data

---

## Integration Notes

**Model:** gpt-4.1 (sync, < 1 min - this is just routing logic)

**Purpose:** This step is orchestration, NOT content generation. It analyzes available data and creates a plan for which sections to generate.

**Next Steps:**
- Routing plan flows to section writers
- Writers with status=GENERATE proceed with content creation
- Writers with status=PLACEHOLDER create brief "pending" notes
- SOURCES writer runs last (after all other sections complete)

**Parallel Execution:**
- Most sections can be generated simultaneously (same inputs: merged_claims)
- Make.com can call all section writer subscenarios in parallel for speed
- SOURCES writer should run sequentially after others (captures all citations)

**Placeholder Strategy:**
- Don't fail pipeline if data is missing - create placeholders
- Placeholders note what's missing and suggest follow-up steps
- User can see incomplete sections and gather missing data
- Dossier still deliverable even if not 100% complete
